...
public class WordMaker 
{
	...
	public static void main(String[] args) throws Exception
	{
		...
		FileInputStream ExcelFileToRead = new FileInputStream("Z:\\Pending Preliminary Listings\\FADI Excel Listings/1680_WellerMichael_20170711.xlsx"); //selects target Excel file to be read
            ...
            while(row.getCell(cont1Start).getNumericCellValue() == boxTracker) 
            {
        		...
        		while(row.getCell(series).getStringCellValue().equals(seriesTracker)) 
            	{
            		...
            		while(row.getCell(subseries).getStringCellValue().equals(subseriesTracker)) 
            		{
            			...
            			{	
            				String begDat = row.getCell(beginDate).toString();
            				String endDat = row.getCell(endDate).toString();
            				SimpleDateFormat slashForm = new SimpleDateFormat("mm/dd/yyyy");
            				SimpleDateFormat dashForm = new SimpleDateFormat("mm-dd-yyyy");
            				try 
            				{
            					headerAndDetails = headerAndDetails + "; " + slashForm.format(dashForm.parse(begDat));
            					if(!begDat.equals(endDat) && !endDat.isEmpty())
                					headerAndDetails = headerAndDetails + "-" + slashForm.format(dashForm.parse(endDat));
            					headerAndDetails = headerAndDetails + ".";
            				}
            				catch(ParseException parse)
            				{
            					if(begDat.substring(begDat.length() - 2).equals(".0"))
            						begDat = begDat.substring(0, begDat.length() - 2);
            					if(!endDat.isEmpty() && endDat.substring(endDat.length() - 2).equals(".0"))
            					{
            						endDat = endDat.substring(0, endDat.length() - 2);
            					}
            					headerAndDetails = headerAndDetails + "; " + begDat;
            					if(!begDat.equals(endDat) && !endDat.isEmpty())
                					headerAndDetails = headerAndDetails + "-" + endDat + ".";
            					else
            						headerAndDetails = headerAndDetails + ".";
            				}		
            			}
            			...
            			{
            				subsubLetter = 97;
            				while(subsubseries != 1 && row.getCell(heading).getStringCellValue().equals(headingTracker))
                			{
                				makeNewRun("L", 4, 0);
		...
		try
		{
			FileOutputStream out = new FileOutputStream(new File("C:\\Users\\student\\Desktop\\Hatchet/target3.docx"));
			doc.write(out);
			out.close();
		}
		catch(Exception e)
